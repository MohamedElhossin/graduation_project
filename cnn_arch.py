...
from tensorflow.keras.layers import Dense, Dropout, Activation, Flatten, Input, ZeroPadding2D
#from keras.layers import Conv2D, MaxPooling2D
from tensorflow.keras.layers import Conv2D,  MaxPooling2D

# Load the data set
hdf5_file = h5py.File ('/content/gdrive/My Drive/gd.hdf5','r')
...
yt = Y_train.T
Y_train_label = np.zeros((yt.shape[1], 3))
...
a = Y_val.T
Y_val_label = np.zeros((a.shape[1], 3))
...
model = Model(inputs=X_input, outputs=model, name='mymodel')

#Compile the model

model.compile(optimizer='adam', loss='categorical_crossentropy', metrics=['accuracy'])

model.fit(X_train, Y_train_label, epochs=20, batch_size=16)

#Evaluate model
model.evaluate(X_val, Y_val_label)

model.save('/content/gdrive/My Drive/gd/mymodel.h5')
